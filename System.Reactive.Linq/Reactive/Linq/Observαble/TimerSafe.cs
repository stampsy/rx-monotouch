...
namespace System.Reactive.Linq.Observαble
{
    class TimerSafe : Producer<Unit>
    {
        ...
        public TimerSafe(DateTimeOffset dueTime, TimeSpan? period, IScheduler scheduler)
        {
            _dueTimeA = dueTime;
            _period = period;
        ...
        public TimerSafe(TimeSpan dueTime, TimeSpan? period, IScheduler scheduler)
        {
            _dueTimeR = dueTime;
            _period = period;
            _scheduler = scheduler;
        ...
        protected override IDisposable Run(IObserver<Unit> observer, IDisposable cancel, Action<IDisposable> setSink)
        {
            if (_period.HasValue)
            {
        ...
        class _ : Sink<Unit>
        {
            private readonly TimerSafe _parent;
            
            public _(TimerSafe parent, IObserver<Unit> observer, IDisposable cancel)
            ...
            private void Invoke()
            {
                base._observer.OnNext(Unit.Default);
        ...
        class π : Sink<Unit>
        {
            ...
            public π(TimerSafe parent, IObserver<Unit> observer, IDisposable cancel)
                : base(observer, cancel)
            {
                _parent = parent;
            ...
            public IDisposable Run()
            {
                ...
                else
                {
                    ...
                    if (dueTime == _period)
                    {
                        return _parent._scheduler.SchedulePeriodic(Unit.Default, _period, (Func<Unit, Unit>)Tick);
                    }
